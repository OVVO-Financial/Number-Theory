// C++17 GMP parallel staged bottom-start multi-seed batch-GCD accelerator
//
...
// g++ -std=c++17 -O3 paper_test_gmp_parallel_staged_sqrtb.cpp -lgmpxx -lgmp -pthread -o paper_test_gmp_parallel_staged_sqrtb.exe
//
// Run:
// ./paper_test_gmp_parallel_staged_sqrtb.exe
//
// Arguments:
// ./paper_test_gmp_parallel_staged_sqrtb.exe <base_window_steps> <seed_count> <batch_size> <max_k> <thread_count> <seed_start> <digit_mode> <centered> <schedule_mode> <low_k_max> <b_cap_mode> <target_divisor> <manual_b_cap> <window_step_cap>
//
// digit_mode:
...
//   2 = manual_b_cap
//
// window_step_cap = 0 means no step cap. Use with care for large N.
...
static std::vector<AcceleratorStream> build_streams_for_seed_range(
    ...
    for (std::uint64_t local_seed_index = seed_begin;
        ...
        for (int k = 1; k <= max_k; ++k) {
            std::uint64_t stream_window = adaptive_window_for_k(
                b_cap,
                k,
                base_window_steps,
                schedule_mode,
...
int main(int argc, char** argv) {
    ...
    int schedule_mode               = parse_int_arg(argc, argv, 9, 1);
    ...
    if (schedule_mode < 0 || schedule_mode > 1) {
        std::cerr << "schedule_mode must be 0 or 1.\n";
        return 1;
    }
