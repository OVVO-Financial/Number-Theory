...
Below is the complex space associated with `n=309`.  Each triangle represents a complex number whose lower [corresponding real number](Number%20Theory%20Papers/Real_Pair_Representation_of_Complex_Numbers_and_Quaternions.pdf) lies within the interval `[3,sqrt(n)]`.
...
![Complex Space](Images/Complex%20plane.png)
...
![Trial Div](Images/Trial%20Division%20by%203.jpeg)
...
#### Vertical Fermat
![Fermat](Images/Vertical%20Fermat.jpeg)

#### Horizontal Fermat
![H Fermat](Images/Horizontal%20Fermat.jpeg)


### Combined methods
The above images explain why combining trial division and Fermat's method is more effective than either on its own (while each has the ability to be sieved, namely primes only for trial division and the [Fermat sieve here](Number%20Theory%20Papers/Fermat%20Sieve%20Using%20Complex%20Numbers.pdf)).  However, we are still left with the middle section of the strip to navigate.  [Complex trial multiplication](Prime%20Factorization/Complex%20Trial%20Multiplication.md) is most effective in this area, permitting us to [combine all 3 methods](julia/Simultaneous_Complex_Factorization.jl) to simultaneously navigate their most effective part of the factor strip.
...
![Simultaneous](Images/Complex%20Space%20Factorization%201.jpeg)
...
Also, the [Fermat sieves](Number%20Theory%20Papers/Fermat%20Sieve%20Using%20Complex%20Numbers.pdf) for `a` and `b` are still applicable.

The green vertical line, formerly our factor strip, is now simply our real number `n=309`!

![Squared complex space](Images/Complex%20squared.png)
...
The above plots were generated with the [complex space generator](R/Complex%20Space%20Generator.R) in R.
